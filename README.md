...
| `AI_ASSIST_PROVIDER` / `AI_ASSIST_KEY` | Liga a IA. `gemini` (Google) ou `anthropic` (Claude Haiku — chave em console.anthropic.com, créditos pré-pagos via cartão). Com IA ligada o bot vira AGENTE: busca produtos, manda fotos, monta sacola, salva lead e chama atendente sozinho |
| `AI_ASSIST_MODEL` | `gemini-2.5-flash` ou `claude-haiku-4-5` conforme o provider |
| `AI_ASSIST_MODEL_FALLBACK` | Só Gemini: modelo reserva quando o principal falha (default `gemini-2.0-flash`) |
| `GEMINI_KEY` | Só com provider anthropic: chave Gemini separada pra transcrição de áudio (opcional; sem ela áudio é ignorado) |
